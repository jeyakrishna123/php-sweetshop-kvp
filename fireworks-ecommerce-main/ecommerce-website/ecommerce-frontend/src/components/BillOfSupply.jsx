...
const BillOfSupply = ({ order, onClose }) => {
  ...
  const sendEmailToCustomer = async () => {
    try {
      ...
      const response = await orderAPI.sendBillEmail(order._id);
      
      if (response.success) {
        showToast("📧 Bill email sent successfully to customer!", "success", 3000);
      } else {
        showToast("Failed to send email: " + response.message, "error", 3000);
      }
    } catch (error) {
      console.error("Error sending email:", error);
      showToast("Failed to send email: " + error.message, "error", 3000);
    } finally {
  ...
  const handlePrint = async () => {
    ...
    if (printContent) {
      ...
      // Create the HTML document for the new window
      const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
  ...
  const handleDownload = async () => {
    ...
    if (printContent) {
      ...
      // Wait for content to load, then trigger automatic download
      printWindow.onload = () => {
        setTimeout(() => {
          // Trigger print dialog immediately for PDF download
          printWindow.print();
        ...
        <div className="flex justify-between items-center p-4 border-b bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-t-lg no-print">
          ...
          <div className="flex space-x-2">
            <button
              ...
              className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition duration-200 flex items-center space-x-2 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <span>{isSendingEmail ? "Sending Email..." : "Download & Email PDF"}</span>
            </button>
            <button
              onClick={handlePrint}
              disabled={isSendingEmail}
              className="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition duration-200 flex items-center space-x-2 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
              </svg>
              <span>{isSendingEmail ? "Sending Email..." : "Print & Email"}</span>
            </button>
            <button
              onClick={onClose}
